...
#Oxana RDS source
#solar_data <- readRDS('/Users/opanvas/Downloads/GitHub/R_solar_dataset/data/solar_dataset.RData')
```
```{r, echo=F}
###prepare dataset
...
# Here we are plotting each station to its location on a map
with a color code corresponding to it's altitude 
...
```
# Here we are plotting each station to its location on a map
with a color code corresponding to it's average over time 
...
We want to get rid of extreme outliers and of course first, we have to find them 
...
#will work more closely on both this and outliers later, for now just mapping out sections. And for now I can't knit because of data (will ask in the chat) so it is approximate /yes maybe code is wrong, WILL FIX/
...
Now that we have a good idea of our data, we will start processing it, in order to prepare it for later on building the model.
